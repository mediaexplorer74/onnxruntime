...
namespace Microsoft.ML.OnnxRuntime.InferenceSample
{
    public class InferenceSampleApi : IDisposable
    {
        ...
        public void Execute()
        {
            ...
            using (IDisposableReadOnlyCollection<OrtValue> results = _inferenceSession.Run(new RunOptions() { },
                _orderedInputNames, _inputData, _inferenceSession.OutputNames)  )
            {
                // dump the results
                for (int i = 0; i < results.Count; ++i)
                {
                    var name = _inferenceSession.OutputNames[i];
                    Debug.WriteLine("Output for {0}", name);
